...
function hienThiDoiTuongTheoId(idCanTim,so) {
  ...
  for (let i = 0; i < yngia.length; i++) {
    ...
    //         arr = arr.slice(2, arr.length == 10 ? 10 : 8); // Lấy từ ký tự thứ 2 đến ký tự thứ 10 nếu arr.length == 10, ngược lại lấy từ ký tự thứ 2 đến ký tự thứ 8
    //         const variable1 = `${parseInt(arr[0]) > 8 ? parseInt(arr[0]) - 8 : parseInt(arr[0]) === 0 ? 8 : parseInt(arr[0])}/${parseInt(arr[1]) > 8 ? parseInt(arr[1]) - 8 : parseInt(arr[1]) === 0 ? 8 : parseInt(arr[1])}`;
    //         const variable2 = `${parseInt(arr[2]) > 8 ? parseInt(arr[2]) - 8 : parseInt(arr[2]) === 0 ? 8 : parseInt(arr[2])}/${parseInt(arr[3]) > 8 ? parseInt(arr[3]) - 8 : parseInt(arr[3]) === 0 ? 8 : parseInt(arr[3])}`;
    //         const variable3 = `${parseInt(arr[4]) > 8 ? parseInt(arr[4]) - 8 : parseInt(arr[4]) === 0 ? 8 : parseInt(arr[4])}/${parseInt(arr[5]) > 8 ? parseInt(arr[5]) - 8 : parseInt(arr[5]) === 0 ? 8 : parseInt(arr[5])}`;
    ...
    // phần 2 
    function showInputValue1() {
        // Lấy thẻ input theo class
        var inputElement = document.querySelector('.chuoi1');
       ...
       //console.log("độ dài chuỗi : "+ inputValue.length);
        ...
        var dt = inputValue;
        console.log("data truyền vào : "+ dt+"kiểu :"+typeof(dt));
    ...
    console.log("Biến 4 : "+bien4);

        console.log("Kiểu  mang 4: "+ typeof(mang_4_phan_tu)+"Giá trị : "+mang_4_phan_tu);
        
        console.log(kiemTraVaTaoMang(bbdn,mang_4_phan_tu));
        var mangmoi = kiemTraVaTaoMang(bbdn,mang_4_phan_tu);
       console.log("Mảng mới : "+mangmoi);
...
// Làm tròn phần thập phân và tính kết quả cuối cùng
 var kqq = Math.round(parseFloat(`0.${decimalPart || 0}`) * 80);
        var mucdo1;
        var nghia1;
        const ynssDiv = document.querySelector('.ynss');
        const yndsDiv = document.querySelector('.ynds');
        
        // Xóa nội dung cũ trong các thẻ div
        ynssDiv.innerHTML = "";
        yndsDiv.innerHTML = "";
      List64.forEach(item => {
        if (item.id === kqq) {
            ...
            ynssDiv.innerHTML += `<p>${nghia}</p>`;
            yndsDiv.innerHTML += `<p>${mucdo}</p>`;
...
// }
    ...
    // Hiển thị giá trị trong console
       ...
       console.log("aa: "+dt.length);
       console.log("đấy : "+splitArray(dt));
       
       
    // const [result1, result2, result3] = splitArray(bien2);
    const [result1,result2,result3, result4]  =  processArray(dt);
    console.log("dayyy " +processArray(dt));
    console.log("sau khi cắt : "+result1,result2,result3,result4);
...
//         newRows += newRow; // Tích lũy các hàng mới vào biến newRows
//     });

   // BangCuoi.innerHTML = newRows;
...
QDich.forEach(function(item) {
    if (result3 == item.Stt && result4 == item.Stt) {
...
//          // Kiểm tra nếu k không vượt quá độ dài của mảng mangmoi
//             // var mmm = mangmoi[k++].tn;
//             let newRow = `
//                 <tr>
//                     <td>${type}</td>
//                     <td></td>
...
//     });
// });
for (let i = 0; i < ["Sơ Niên Vận", "Tiền Vận", "Trung Vận", "Hậu Vận"].length; i++) {
    const type = ["Sơ Niên Vận", "Tiền Vận", "Trung Vận", "Hậu Vận"][i];
    dataByType[type].forEach(function(item) {
        if (k < mangmoi.length) { // Kiểm tra nếu k không vượt quá độ dài của mảng mangmoi
            var mmm = mangmoi[k++].tn;
            let newRow = `
                <tr>
                    <td>${type}</td>
...
var ns = document.querySelector('#born');
...
const tuoiDiv = document.querySelector('.tuoi');
const vanDiv = document.querySelector('.van');
const dichDiv = document.querySelector('.dich');
...
console.log(typeof(arr) +'-'+typeof(menhc));
console.log(arr);
snvan(arr,menhc);
var arrNew = snvan(arr,menhc);
console.log("hihi : "+arrNew[0]);
console.log("huhu : "+ arrNew[7]);
var dg = DienGiai(nsinh);

tuoiDiv.innerHTML = `<p>${tuoi}</p>`;
vanDiv.innerHTML = `<p>${vanht}</p>`;
dichDiv.innerHTML = `<p>${dg}</p>`;


var snvDiv = document.querySelector(".snv");
    var tvDiv = document.querySelector(".tv");
    var trvDiv = document.querySelector(".trv");
    var hvDiv = document.querySelector(".hv");

    // Xóa dữ liệu cũ trong các thẻ div
    snvDiv.textContent = "";
    tvDiv.textContent = "";
    trvDiv.textContent = "";
    hvDiv.textContent = "";

snvDiv.innerHTML = `${arrNew[0]} - ${arrNew[1]}`;
tvDiv.innerHTML = `${arrNew[2]} - ${arrNew[3]}`;
trvDiv.innerHTML = `${arrNew[4]} - ${arrNew[5]}`;
hvDiv.innerHTML = `${arrNew[6]} - ${arrNew[7]}`;
